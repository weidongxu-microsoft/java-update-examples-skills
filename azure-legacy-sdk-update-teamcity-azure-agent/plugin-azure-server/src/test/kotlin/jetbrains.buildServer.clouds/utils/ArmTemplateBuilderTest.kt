...
class ArmTemplateBuilderTest {
    ...
    fun testSetPublicIpWithNewSKU() {
        ...
        Assert.assertEquals(builder.toString(),
            "{\"variables\":{\"pipName\":\"[concat(parameters('vmName'), '-pip')]\"},\"resources\":[{\"name\":\"" +
                    "[variables('nicName')]\",\"properties\":{\"ipConfigurations\":[{\"properties\":" +
                    "{\"publicIPAddress\":{\"id\":\"[resourceId('Microsoft.Network/publicIPAddresses', " +
                    "variables('pipName'))]\"}}}]},\"dependsOn\":[\"[concat('Microsoft.Network/publicIPAddresses/', " +
                    "variables('pipName'))]\"]},{\"apiVersion\":\"2024-10-01\",\"type\":" +
                    "\"Microsoft.Network/publicIPAddresses\",\"name\":\"[variables('pipName')]\",\"location\":" +
                    "\"[variables('location')]\",\"sku\":{\"name\":\"Standard\"},\"properties\":" +
                    "{\"publicIPAllocationMethod\":\"Static\",\"deleteOption\":\"Delete\"}}]}")
    }
    ...
    @Test(dataProvider = "evictionPolicies")
    fun testSpotInstanceEvictionPolicyWithNoMaxPrice(evictionPolicy: String) {
        val builder = ArmTemplateBuilder("""{"resources": [
            {
                "type": "Microsoft.Compute/virtualMachines",
                "name": "myName",
        ...
        ]}""").setupSpotInstance(false, null, evictionPolicy)

        Assert.assertEquals(builder.toString(),
            "{\"resources\":[{\"type\":\"Microsoft.Compute/virtualMachines\",\"name\":\"myName\",\"properties\":{\"priority\":\"Spot\",\"evictionPolicy\":\"${evictionPolicy}\",\"billingProfile\":{\"maxPrice\":-1}}}]}")
    }

    @Test(dataProvider = "evictionPolicies")
    fun testSpotInstanceEvictionPolicyWithMaxPrice(evictionPolicy: String) {
        val builder = ArmTemplateBuilder("""{"resources": [
            {
                "type": "Microsoft.Compute/virtualMachines",
                "name": "myName",
        ...
        ]}""").setupSpotInstance(true, 123, evictionPolicy)

        Assert.assertEquals(builder.toString(),
            "{\"resources\":[{\"type\":\"Microsoft.Compute/virtualMachines\",\"name\":\"myName\",\"properties\":{\"priority\":\"Spot\",\"evictionPolicy\":\"${evictionPolicy}\",\"billingProfile\":{\"maxPrice\":0.00123}}}]}")
    }

    @DataProvider(name = "evictionPolicies")
    fun evictionPolicies(): Array<Array<Any>> {
        return arrayOf(
            arrayOf("Deallocate"),
            arrayOf("Delete")
        )
